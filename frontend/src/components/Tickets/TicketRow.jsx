  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const historialPayload = {
        idHistorialTicket: 0,
        accionTicket,
        comentarios: comment || t("tickets.history.assignedTo", { email: newAssigned ?? "N/A" }),
  ...
  return (
    <>
      <TableRow hover sx={{ "& td": { borderColor: "#e0e0e0" } }}>
        ...
        {isCoordinator && (
          <TableCell>
            {employeeMap?.[ticket.assigned_to] || <i>{t("tickets.row.noAssigned")}</i>}
          </TableCell>
        )}
      </TableRow>
